...
export default function Dashboard() {
  ...
  useEffect(() => { setEntries(loadRecent(30)); }, []);
  ...
  const completion = entries.length
    ? Math.round(entries.reduce((acc: number, e: any) =>
        acc + (e.todos.filter((t: any) => t.checked).length / Math.max(e.todos.length, 1)), 0) / entries.length * 100)
    : 0;
